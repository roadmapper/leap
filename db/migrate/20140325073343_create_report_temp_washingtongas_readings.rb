class CreateReportTempWashingtongasReadings < ActiveRecord::Migration
  def up
    self.connection.execute %Q(CREATE OR REPLACE VIEW temp_washingtongas_readings AS 
        SELECT 
            ...
            WHERE
                record_lookups.company_name = 'WASHINGTONGAS'
                    AND properties.finish_date IS NOT NULL;)
  end

  def down
    self.connection.execute "DROP VIEW IF EXISTS temp_washingtongas_readings;"
  end
end
